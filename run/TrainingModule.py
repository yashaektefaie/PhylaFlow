...
class TrainingModule(LightningModule):
    ...
    def forward(self, batched_tokenized_trees, t, phyla_embeddings):
    ...
    def step(self, batch, eval=False):
        ...
        v_pred, edge_split_masks, edge_mask = self.forward(
            batch["tokenized_trees"], batch["batched_time"], batch["phyla_embeddings"]
        )
        ...
        for num in range(len(velocity_labels)):
            ...
            for vel in velocity_labels[num]:
                ...
                if vel not in edge_split_masks[num]:
                    raise Exception(
                        f"Split {vel} from velocity labels not found in edge splits {edge_split_masks[num]}!"
                    )

                sub_gathered_velocity_labels.append(velocity_labels[num][vel])
                sub_v_pred_indices.append(edge_split_masks[num].index(vel))

            if len(sub_v_pred_indices) > 0:
                batch_indices = torch.tensor(
                    sub_v_pred_indices, device=v_pred.device, dtype=torch.long
                )
                ...
                # v_pred[num] is (SeqLen, 1), index_select gives (K, 1), squeeze gives (K)
                batch_preds = v_pred[num].index_select(0, batch_indices).squeeze(-1)

                gathered_velocity_labels.append(batch_targets)
                v_pred_indices.append(batch_preds)

        if len(v_pred_indices) > 0:
            flat_preds = torch.cat(v_pred_indices)
            flat_targets = torch.cat(gathered_velocity_labels)
            loss = ((flat_preds - flat_targets) ** 2).mean()
        else:
