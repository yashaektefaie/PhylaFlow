...
RUN_NAME="realstream_leafcap120_12451_phy256_leafglobal_cladehead_metricprobe64_fh16_aradd_mlp2_s128_lr2e3_ds2eval_replaysubset_mrbayes20k_20260504"
WORKDIR="/home/unix/yektefai/PhylaFlow"
CONFIG_PATH="/ewsc/yektefai/30272299/launch_configs_ewsc/configs/local_realstream_348299_phy256_leafglobal_cladehead_metricprobe64_fh16_aradd_mlp2_s128_lr2e3_ds2eval_replaysubset_mrbayes20k_20260504.yaml"
...
export TMPDIR="${TMPDIR:-$EWSC_ROOT/tmp}"
export TMP="${TMP:-$TMPDIR}"
export TEMP="${TEMP:-$TMPDIR}"
export WANDB_DIR="${WANDB_DIR:-$EWSC_ROOT/wandb}"
export WANDB_CACHE_DIR="${WANDB_CACHE_DIR:-$EWSC_ROOT/wandb_cache}"
export WANDB_DATA_DIR="${WANDB_DATA_DIR:-$EWSC_ROOT/wandb_data}"
export WANDB_ARTIFACT_DIR="${WANDB_ARTIFACT_DIR:-$EWSC_ROOT/wandb_artifacts}"
export WANDB_CONFIG_DIR="${WANDB_CONFIG_DIR:-$EWSC_ROOT/wandb_config}"
export XDG_CACHE_HOME="${XDG_CACHE_HOME:-$EWSC_ROOT/cache}"
export TORCH_HOME="${TORCH_HOME:-$EWSC_ROOT/torch_home}"
export TORCH_EXTENSIONS_DIR="${TORCH_EXTENSIONS_DIR:-$EWSC_ROOT/torch_extensions}"
export TRITON_CACHE_DIR="${TRITON_CACHE_DIR:-$EWSC_ROOT/triton/manual}"
export MPLCONFIGDIR="${MPLCONFIGDIR:-$EWSC_ROOT/mpl_config}"
...

  {
    tr '\0' '\n' < "/proc/${pid}/environ" |
      awk -F= '
        ...
        $1 == "WANDB_CACHE_DIR" ||
        $1 == "WANDB_DATA_DIR" ||
        $1 == "WANDB_ARTIFACT_DIR" ||
        $1 == "WANDB_CONFIG_DIR" ||
        $1 == "XDG_CACHE_HOME" ||
        $1 == "TORCH_HOME" ||
        $1 == "TORCH_EXTENSIONS_DIR" ||
        $1 == "TRITON_CACHE_DIR" ||
...
6. Relaunch with temporary files and W&B/cache directories on /ewsc. Prefer scripts/launch_phylaflow_ewsc_safe.sh when possible.
7. Write a concise incident report to ${incident_dir}/codex_incident_report.md with cause, files changed, verification, and the relaunch command/PID.
